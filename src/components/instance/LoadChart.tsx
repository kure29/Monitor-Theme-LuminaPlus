...
import type { LoadRecord, NodeMetrics } from "@/types/models";
...
export function LoadChart({
  ...
  const historyPoints = useMemo<ChartPoint[]>(() => {
    const rawPoints = historyRecords.map(({ record, time }) => {
      ...
      return {
        ...
        swap:
          record.history_capabilities?.swap === false
            ? null
            : totals.swapTotal > 0
              ? (record.swap / totals.swapTotal) * 100
              : null,
        disk: totals.diskTotal > 0 ? (record.disk / totals.diskTotal) * 100 : null,
        netIn: record.net_in,
        netOut: record.net_out,
        connections: record.history_capabilities?.connections === false ? null : record.connections,
        udp: record.history_capabilities?.connections === false ? null : record.connections_udp,
        process: record.history_capabilities?.process === false ? null : record.process,
      };
    });
  ...
  const latestHistoryTotals = latestHistoryRecord
    ...
    : null;
  const historyCapabilities = latestHistoryRecord?.history_capabilities;
    ...
    >
      <div className="instance-chart-grid">
          ...
          note={
            isRealtime && node
              ? node.swapTotal
                ...
                : "Swap 无"
              : historyCapabilities?.swap === false
                ? "Swap 历史不可用"
        ...
        <ChartCard
          ...
          note={historyCapabilities?.trafficTotals === false && !isRealtime
            ? "仅提供速率历史"
            : (
          ...
          xRange={requestedXRange}
        />
        {(isRealtime || historyCapabilities?.connections !== false) && <ChartCard
          ...
          xRange={requestedXRange}
        />}
        {(isRealtime || historyCapabilities?.process !== false) && <ChartCard
          icon={<Gauge size={13} />}
          title="进程"
          uuid={uuid}
          value={
