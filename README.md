...
- 首页 Ping 需要在主题设置中选择 monitor 已分配给节点的探测任务；设置页可从后台分配一键同步，
  节点详情页可直接读取已分配探测任务的历史。
...
- **卡片只显示你在主题设置里选定的任务**。后台分配决定哪些节点真的会探测；设置页第 9 节
  的「同步后台分配」可补齐仅有一项任务的节点，并保留仍有效的原有选择；有多个任务且尚未
  选择的节点需手动确定线路。按任务「全选可用」时，只选择后台已分配且符合当前搜索的节点；
  原来选在其他首页任务的节点会移到当前任务。
  没有选择的节点显示「未配置」。三网模式可用三项全局任务覆盖全部节点；如果后台尚未将某项任务
  分配给某台节点，首页会标出「未分配」，设置页也会显示受影响的节点数。此时需先在 monitor
  后台分配任务，主题设置里的勾选不会代替后台分配。
